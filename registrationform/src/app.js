...
app.post('/empdata',[
 ...
 try{
   ...
   // check user is exist or not 
    ...
    {
         success=false;
       return res.status(404).json(success,"user already exists");
...
app.post('/login', [
  ...
  console.log(user);
   ...
   {
      success =false
      return res.status(400).json({ errors: "sorry login with perfect email"});
...
if(!passwordCompare) 
{
   ...
   success =false  
   return res.status(400).json({success, errors: "sorry login with perfect password "});
}
 else{
